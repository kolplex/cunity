...
class Skoch_Filter_File_Crop implements \Zend_Filter_Interface {
    ...
    public function __construct($options = []) {
        ...
        if (isset($options['adapter'])) {
            if ($options['adapter'] instanceof \Skoch_Filter_File_Adapter_Abstract) {
                $this->_adapter = $options['adapter'];
            } else {
                $name = $options['adapter'];
                if (substr($name, 0, 26) != 'Skoch_Filter_File_Adapter_') {
                    $name = 'Skoch_Filter_File_Adapter_' . ucfirst(strtolower($name));
                }
                $this->_adapter = $name;
            }
        }
    ...
    public function filter($value) {
        if ($this->_directory) {
            $target = $this->_directory . '/' . $this->_prefix . basename($value);
        } else {
        ...
        return $this->_adapter->crop($this->_x, $this->_y, $this->_x1, $this->_y1, $value, $target, $this->_thumbwidth);
    }
