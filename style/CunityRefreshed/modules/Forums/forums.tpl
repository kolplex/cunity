<div class="page-buttonbar clearfix">
    ...
    {-if $user.groupid > 1}<button class="pull-right btn btn-primary" data-toggle="modal" data-target="#addForum" data-forumid="{%=o.id%}"><i class="fa fa-plus"></i>&nbsp;{-"New Forum"|translate}</button>{-/if}
</div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="createForumForm" class="form-horizontal ajaxform" role="form" action="{-"index.php?m=forums&action=createForum"|URL}" method="post">
                    <div class="form-group">
                        <label for="forum-title" class="col-sm-2 control-label">{-"Title"|translate}</label>
                        <div class="col-sm-10">
                            <input type="text" name="title" id="forum-title" class="form-control" placeholder="{-""|translate}">
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="forum-content" class="col-sm-2 control-label">{-"Description"|translate}</label>
                        <div class="col-sm-10">
                            <textarea class="form-control" id="forum-content" name="description"></textarea>
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="col-sm-offset-2 col-sm-10">
                            <div class="checkbox">
                                <label>
                                    <input type="checkbox" name="board_permissions" value="1">&nbsp;{-"Users are allowed to create new boards"|translate}
                                </label>
                            </div>
                        </div>
                    </div>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" data-dismiss="modal" onclick="$('#createForumForm').submit();">{-"Create"|translate}</button>
            </div>
        </div>
    </div>
</div>
<script id="forum-template" type="text/html">
    <div class="panel panel-default forum" id="forum-{%=o.id%}">
        <div class="panel-heading clearfix">
            <h4 class="pull-left"><a href="{%=convertUrl({'module':'forums','action':'forum','x':o.id})%}">{%=o.title%}</a></h4>
        </div>
...
<script id="board-template" type="text/html">
    <div class="list-group-item media">
        ...
        <div class="media-body">
            <a href="{%=convertUrl({'module':'forums','action':'board','x':o.id})%}"><h4 class="list-group-item-heading">{%=o.title%}</h4></a>
