<div class="page-buttonbar clearfix">
    ...
    <button class="btn btn-primary pull-right" data-toggle="modal" data-target="#messages_modal"><i class="fa fa-pencil"></i>&nbsp;{-"Start Conversation"|translate}</button>
</div>
...
<script id="inbox-conversation" type="text/html">
    <article class="inbox-item clearfix">
        ...
        <div class="inbox-item-content pull-left">
            <h2>{% if (o.status == 1) { %}<span class="label label-primary">{-"new"|translate}</span>&nbsp;{% } %}<a href="{-"index.php?m=messages&action="|URL}{%=o.conversation%}">{%=o.name%}</a></h2>
