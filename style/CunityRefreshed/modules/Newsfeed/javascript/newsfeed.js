...
$(document).ready(function() {
    ...
    $(document).on("click", ".newsfeed-post-area,.newsfeed-post-option", function() {
        ...
        sendRequest({ref_name: $(this).data("type"), ref_id: $(this).data("refid"), last: $("#comments-" + postid + " .post-comment:last").data("id"), limit: 10}, "comments", "get", function(res) {
        ...
        if (youtubereg !== null) {
            ...
            $.getJSON("http://gdata.youtube.com/feeds/api/videos/" + vid + "?v=2&alt=json", function(data) {
                ...
                $(".newsfeed-newpost input[name='youtubedata']").val(JSON.stringify({vid: vid, title: data.entry.title.$t, description: data.entry.media$group.media$description.$t.substring(0, 200), link: data.entry.link[0].href, thumbnail: data.entry.media$group.media$thumbnail[1].url}));
            });
        } else {
...
function load(refresh, filter) {
    ...
    sendRequest({wall_owner_id: $("#newsfeed-owner-id").val(), wall_owner_type: $("#newsfeed-owner-type").val(), offset: offset, refresh: refreshId, filter: filter}, "newsfeed", "load", function(res) {
