...
<div class="profile-banner{-if $profile.timg eq null} profile-banner-empty{-/if}" style="background-image: url('cr_{-$profile.timg}');">
    ...
    {-if $user.userid != $profile.userid}
        <div class="nofriends-buttons btn-group friend-buttons{-if $profile.status !== null} hidden{-/if}">                   
            <button class="btn btn-default" data-userid="{-$profile.userid}" data-action="addasfriend" data-toggle="modal" data-target="#relationship-modal"><span class="fa fa-plus"></span> {-"Add as friend"|translate}</button>
        ...
        <div class="blocked-buttons btn-group friend-buttons{-if $profile.status != 0 || $profile.sender != $user.userid} hidden{-/if}">                   
            <button class="btn btn-default tooltip-trigger" data-title="{-"You have blocked this user"|translate}" data-container="body"><span class="fa fa-ban"></span> {-"Blocked"|translate}</button>
            <button class="btn btn-default dropdown-toggle" data-toggle="dropdown"><span class="caret"></span></button>
            <ul class="dropdown-menu">                                                
                <li><a href="#" data-target="#relationship-modal" data-toggle="modal" data-action="unblock" data-userid="{-$profile.userid}"><i class="fa fa-eraser"></i> {-"Unblock"|translate}</a></li>
            </ul>                                               
        </div>
        <div class="request-buttons btn-group friend-buttons{-if $profile.status != 1 || $profile.sender != $profile.userid} hidden{-/if}">                   
            <button class="btn btn-default" data-userid="{-$profile.userid}" data-action="confirmfriend" data-toggle="modal" data-target="#relationship-modal"><span class="fa fa-question"></span> {-"Answer Request"|translate}</button>
            <button class="btn btn-default dropdown-toggle" data-toggle="dropdown"><span class="caret"></span></button>
            <ul class="dropdown-menu">
                {-if $profile.privacy.message eq 3}
                    <li><a href="javascript:sendMessage('{-$profile.name}',{-$profile.userid});"><i class="fa fa-envelope-o"></i> {-"Send message"|translate}</a></li>
        ...
        <div class="pending-buttons btn-group friend-buttons{-if $profile.status != 1 || $profile.sender == $profile.userid} hidden{-/if}">                   
            ...
            <ul class="dropdown-menu">
                ...
                <li><a href="#" data-userid="{-$profile.userid}" data-action="blockperson" data-toggle="modal" data-target="#relationship-modal"><i class="fa fa-ban"></i> {-"Block Person"|translate}</a></li>
                <li class="divider"></li>
                <li><a href="#" data-userid="{-$profile.userid}" data-action="removerequest" data-toggle="modal" data-target="#relationship-modal"><i class="fa fa-times"></i> {-"Remove request"|translate}</a></li>
            </ul>                    
        </div>
...
<ul class="nav nav-pills profile-menu" id="profile-menu">
    {-if ($profile.privacy.visit eq 1 && $profile.status > 2) || $profile.privacy.visit eq 3 || $profile.userid eq $user.userid}
        <li><a href="#Pins" data-toggle="pill"><i class="fa fa-thumb-tack"></i>&nbsp;{-"Pins"|translate}</a></li>
        {-/if}
    <li class=""><a href="#Wall" data-toggle="pill"><i class="fa fa-rss"></i>&nbsp;{-"Wall"|translate}</a></li>
    <li class=""><a href="#Friends" data-toggle="pill" onclick="loadFriends({-$profile.userid});"><i class="fa fa-users"></i>&nbsp;{-"Friends"|translate}&nbsp;<span class="badge">{-if $profile.friendscount > 0}{-$profile.friendscount}{-/if}</span></a></li>
    <li class=""><a href="#Photos" data-toggle="pill" onclick="loadAlbums({-$profile.userid});"><i class="fa fa-picture-o"></i>&nbsp;{-"Photo Albums"|translate}&nbsp;<span class="badge">{-if $profile.albumscount > 0}{-$profile.albumscount}{-/if}</span></a></li>
            {-*<li class=""><a href="#Files" data-toggle="pill"><i class="fa fa-folder-open-o"></i>&nbsp;{-"Files"|translate}</a></li>
            <li class=""><a href="#Events" data-toggle="pill"><i class="fa fa-calendar"></i>&nbsp;{-"Events"|translate}</a></li>*}
            {-if $user.userid eq $profile.userid}
        <li class="pull-right hidden-sm hidden-xs"><a href="{-"index.php?m=profile&action=edit"|URL}" class="tab-no-follow"><i class="fa fa-wrench"></i>&nbsp;{-"Edit profile"|translate}</a></li>
        {-/if}
</ul>
