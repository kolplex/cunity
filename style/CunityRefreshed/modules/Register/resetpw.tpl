...
{-if $success}
    <div class="alert alert-block alert-success"><h4 class="alert-heading">{-"Done!"|translate}</h4><p>{-"You've changed your password successfully! You can now login"|translate}</p></div>
        {-else} 
    <form class="form-horizontal reset-form" action="{-"index.php?m=register&action=reset"|URL}" method="POST" onsubmit="return checkRegistration();">
        ...
        <div class="form-group {-if !empty($error_messages.token)}has-feedback has-error{-/if}">
            ...
            <div class="col-lg-6">
                <input type="number" class="form-control" id="input-token" name="token" value="{-if $smarty.get.x eq ""}{-$values.token}{-else}{-$smarty.get.x}{-/if}">
                <span class="fa fa-exclamation-triangle form-control-feedback"></span>
